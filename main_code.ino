...
String myDeploymentID = ""; //add your deployment id here
String myMainFolderName = "ESP32-CAM";
//======================================== 

//Variables for Timer/Millis.
unsigned long previousMillis = 0; 
const int Interval = 60000; //--> Capture and Send a photo every 60 seconds(u can change as your requirement).
...
void setup() {
  ...
  config.pixel_format = PIXFORMAT_JPEG; //this is for esp32_cam module OV2640 and above. If below specs--> not supported for jpeg format
  
  // init with high specs to pre-allocate larger buffers
  if(psramFound()){
    config.frame_size = FRAMESIZE_UXGA; //can set resolution as your requirement
    config.jpeg_quality = 10;  //0-63 lower number means higher quality
    config.fb_count = 2;
  } else {
    config.frame_size = FRAMESIZE_QVGA; //can set resolution as your requirement
  ...
  s->set_framesize(s, FRAMESIZE_SXGA);  //--> UXGA|SXGA|XGA|SVGA|VGA|CIF|QVGA|HQVGA|QQVGA #any resolution can be setted as per your requirements

  Serial.println("Setting the camera successfully.");
  Serial.println();

  delay(1000); //delay for capturing and uplaoding interval
